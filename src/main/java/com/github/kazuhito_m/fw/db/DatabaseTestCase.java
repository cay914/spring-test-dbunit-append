...
public abstract class DatabaseTestCase {
     ...
     * ロガー(このクラスの時のみ)
     */
    protected final Logger selfLogger = LoggerFactory.getLogger(DatabaseTestCase.class);
     ...
     *
     * @param keyword    そのエビデンスの意味。ファイル名の一部に使われる。
     * @param tableNames 「出力したいテーブル名」の文字列配列。
     */
    public void getEvidence(String keyword, String... tableNames) {
        ...
        try {
            ...
            File outFile = new File(makeTestMethodEvidenceDir(), createExcelFileName(keyword));
     ...
     */
    protected IDatabaseConnection getDBUnitCurrentConnection() throws DatabaseUnitException, SQLException {
        // データソース取得。(プロキシかましてあるから「同一transaction」でデータ取得できる。)
        // 逆にその型じゃなければこけてくれたのむから。
//        TransactionAwareDataSourceProxy dataSource = (TransactionAwareDataSourceProxy) BeanFactory.getBean("dataSource");
...
//        return new DatabaseConnection(dataSource.getConnection());
     ...
     * 出力するExcelファイル名を作成する。
     *
     * @param keyword 名前につけるサフィックス。未指定(null/空)なら数値・日付から自動的に作成される。
    ...
    protected String createExcelFileName(String keyword) {
        ...
        return "evidnece-" + String.format("%1$03d", evidenceCount) + "-" + sufix + ".xlsx";
    }
    ...
    protected File makeTestMethodEvidenceDir() {
        ...
        String evidencePathTop = resourcePath.replaceFirst("test-classes", EVIDENCE_DIR);
        // 最後に、クラス名/メソッド名と付ける。
        File outDir = new File(evidencePathTop, this.getClass().getSimpleName() + "/" + names.getMethodName());
        // ディレクトリを作成しておく。
        if (!outDir.exists())
            outDir.mkdirs();
        return outDir;
